...
async function runAiRead() {
  ...
  try {
    ...
    const out = await callClaude([imageBlock, { type: "text", text: AI_READ_PROMPT }], 1200);
    $("aiReadResult").innerHTML = '<div class="explain-box" style="margin-top:12px;">' + escapeHtml(out || "No response returned.") + '</div>';
  } catch (err) {
