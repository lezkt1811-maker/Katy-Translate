...
function renderBoxes() {
  ...
  state.regions.forEach(region => {
    ...
    box.addEventListener("click", event => {
      ...
      if (state.selectedIds.has(region.id)) state.selectedIds.delete(region.id);
      else state.selectedIds.add(region.id);
      renderBoxes();
      renderSelectionText();
      clearResults();
    });
...
function renderSelectionText() {
  ...
  $("selectionText").innerHTML = text
    ? escapeHtml(text)
    : '<span class="empty">Tap boxes on the image to build a selection.</span>';
}
...
$("scanBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    renderBoxes();
    renderSelectionText();
    setStatus(`Found ${state.regions.length} text region${state.regions.length === 1 ? "" : "s"}. Tap the words you want.`);
  } catch (error) {
    console.error(error);
