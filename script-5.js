...
function handleFile(file) {
  ...
  reader.onload = e => {
    ...
    state.overlayOn = false; state.lineOverlays = [];
    document.querySelectorAll(".line-overlay").forEach(el => el.remove());
    $("overlayToggleBtn").textContent = "🖼️ Show on image"; $("overlayToggleBtn").classList.remove("active");
...
/* ============================================================
   Image overlay — renders the translation directly on the photo,
   like Google's 2020 photo-import view. Groups detected words into
   lines (translating word-by-word loses grammar), translates each
   line with MyMemory (free, no key), samples the actual background
   color around that line to patch over the original text, and draws
   the translation on top, shrinking font size to fit. This is a
   practical approximation of "inpainting" — a flat sampled color
   patch, not true generative background reconstruction, which needs
   heavy ML infrastructure this app doesn't have. Works well on plain
   backgrounds (signs, book pages); textured/busy backgrounds will
   show a visible patch edge.
   ============================================================ */
function groupWordsIntoLines(regions) {
  const sorted = [...regions].sort((a, b) => a.bbox.y0 - b.bbox.y0 || a.bbox.x0 - b.bbox.x0);
  const lines = [];
  sorted.forEach(r => {
    const h = r.bbox.y1 - r.bbox.y0;
    let line = lines.find(l => Math.abs(l.y0 - r.bbox.y0) < h * 0.6);
    if (!line) { line = { y0: r.bbox.y0, words: [] }; lines.push(line); }
    line.words.push(r);
  });
  return lines.map(l => {
    ...
    return {
      bbox: {
        x0: Math.min(...l.words.map(w => w.bbox.x0)), y0: Math.min(...l.words.map(w => w.bbox.y0)),
        x1: Math.max(...l.words.map(w => w.bbox.x1)), y1: Math.max(...l.words.map(w => w.bbox.y1)),
      },
...
function sampleBoxColors(bbox) {
  const img = $("mainImage");
  const canvas = document.createElement("canvas");
  canvas.width = img.naturalWidth; canvas.height = img.naturalHeight;
  const ctx = canvas.getContext("2d");
  ctx.drawImage(img, 0, 0);
  const pts = [[bbox.x0 - 4, bbox.y0 - 4], [bbox.x1 + 4, bbox.y0 - 4], [bbox.x0 - 4, bbox.y1 + 4], [bbox.x1 + 4, bbox.y1 + 4]];
  ...
  return { bg: "rgb(" + r + "," + g + "," + b + ")", text: luminance > 0.55 ? "#111111" : "#f5f5f5" };
}
...
function drawImageOverlay() {
  ...
  const s = scale();
  state.lineOverlays.forEach(o => {
    const div = document.createElement("div");
    div.className = "line-overlay";
    const x = o.bbox.x0 * s, y = o.bbox.y0 * s, w = (o.bbox.x1 - o.bbox.x0) * s, h = (o.bbox.y1 - o.bbox.y0) * s;
    Object.assign(div.style, { left: x + "px", top: y + "px", width: w + "px", height: h + "px", background: o.bg, color: o.color });
    div.textContent = o.text;
    let fontSize = Math.max(9, Math.min(h * 0.7, 22));
    div.style.fontSize = fontSize + "px";
    $("stage").appendChild(div);
    let tries = 0;
    while (div.scrollWidth > w && fontSize > 7 && tries < 20) { fontSize -= 1; div.style.fontSize = fontSize + "px"; tries++; }
  });
}

async function renderImageOverlay() {
  if (!state.regions.length) { setStatus("Scan the image first."); return; }
  const lines = groupWordsIntoLines(state.regions);
  let src = $("sourceLang").value;
  if (src === "auto") {
    const detected = await detectLanguage(lines.map(l => l.text).join(" "));
    src = detected || "en";
  }
  const tgt = $("targetLang").value;
  state.lineOverlays = [];
  for (let i = 0; i < lines.length; i++) {
    setStatus("Translating line " + (i + 1) + " of " + lines.length + " for the image overlay…");
    let translated = lines[i].text;
    try { translated = await myMemoryTranslateChunk(lines[i].text, src, tgt); } catch {}
    state.lineOverlays.push({ bbox: lines[i].bbox, text: translated, ...sampleBoxColors(lines[i].bbox) });
...
$("overlayToggleBtn").addEventListener("click", async () => {
  ...
  $("overlayToggleBtn").textContent = state.overlayOn ? "🖼️ Hide image overlay" : "🖼️ Show on image";
  $("overlayToggleBtn").classList.toggle("active", state.overlayOn);
  if (state.overlayOn) {
    $("overlayToggleBtn").disabled = true;
    if (!state.lineOverlays.length) await renderImageOverlay();
    else drawImageOverlay();
...
$("scanBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    // Import-from-photo behavior: select everything found and translate it
    // immediately — no tapping words first. Free path (OCR + MyMemory),
    // no API key needed. You can still deselect/reselect words afterward
    // to translate just a phrase instead of the whole photo.
    if (state.regions.length > 0) {
      state.selectedIds = new Set(state.regions.map(r => r.id));
      renderBoxes(); renderSelectionText();
      await runTranslate();
    }
...
/* ============================================================
   Local translation — Transformers.js running NLLB-200-distilled
   entirely in the browser via ONNX Runtime + WebAssembly/WebGPU.
   No API key, no server, no cost, ever. Real tradeoff: ~600MB model
   download on first use (cached afterward), and noticeably slower
   than MyMemory, especially without WebGPU support. NLLB uses its
   own language codes (FLORES-200, e.g. "eng_Latn"), not the ISO
   codes used elsewhere in this app, so this table maps the ones
   this app supports that NLLB also supports. Languages not in this
   map fall back to MyMemory automatically.
   ============================================================ */
const NLLB_LANG_MAP = {
  en: "eng_Latn", es: "spa_Latn", fr: "fra_Latn", de: "deu_Latn", it: "ita_Latn",
  pt: "por_Latn", ru: "rus_Cyrl", ar: "arb_Arab", "zh-CN": "zho_Hans", "zh-TW": "zho_Hant",
  ja: "jpn_Jpan", ko: "kor_Hang", hi: "hin_Deva", bn: "ben_Beng", pa: "pan_Guru",
  ta: "tam_Taml", te: "tel_Telu", mr: "mar_Deva", gu: "guj_Gujr", ur: "urd_Arab",
  fa: "pes_Arab", tr: "tur_Latn", vi: "vie_Latn", th: "tha_Thai", he: "heb_Hebr",
  el: "ell_Grek", uk: "ukr_Cyrl", ro: "ron_Latn", nl: "nld_Latn", sv: "swe_Latn",
  no: "nob_Latn", da: "dan_Latn", fi: "fin_Latn", pl: "pol_Latn", cs: "ces_Latn",
  sk: "slk_Latn", hu: "hun_Latn", bg: "bul_Cyrl", sr: "srp_Cyrl", hr: "hrv_Latn",
  sl: "slv_Latn", lt: "lit_Latn", lv: "lvs_Latn", et: "est_Latn", ga: "gle_Latn",
  cy: "cym_Latn", is: "isl_Latn", mt: "mlt_Latn", sq: "als_Latn", mk: "mkd_Cyrl",
  bs: "bos_Latn", ka: "kat_Geor", hy: "hye_Armn", az: "azj_Latn", kk: "kaz_Cyrl",
  uz: "uzn_Latn", mn: "khk_Cyrl", my: "mya_Mymr", km: "khm_Khmr", lo: "lao_Laoo",
  ne: "npi_Deva", si: "sin_Sinh", ml: "mal_Mlym", kn: "kan_Knda", id: "ind_Latn",
  ms: "zsm_Latn", tl: "tgl_Latn", sw: "swh_Latn", am: "amh_Ethi", so: "som_Latn",
  zu: "zul_Latn", xh: "xho_Latn", af: "afr_Latn", yo: "yor_Latn", ig: "ibo_Latn",
  ha: "hau_Latn", ht: "hat_Latn", jv: "jav_Latn", su: "sun_Latn", gl: "glg_Latn",
  eu: "eus_Latn", ca: "cat_Latn", be: "bel_Cyrl", gd: "gla_Latn", yi: "ydd_Hebr",
  ps: "pbt_Arab", sd: "snd_Arab", ku: "kmr_Latn", mg: "plt_Latn", ny: "nya_Latn",
  sm: "smo_Latn", st: "sot_Latn", sn: "sna_Latn", tg: "tgk_Cyrl", tk: "tuk_Latn",
};

let nllbPipelinePromise = null;
function loadNllbPipeline() {
  if (!nllbPipelinePromise) {
    nllbPipelinePromise = import("https://cdn.jsdelivr.net/npm/@huggingface/transformers@3.8.1")
      .then(mod => mod.pipeline("translation", "Xenova/nllb-200-distilled-600M", {
        progress_callback: (p) => {
          if (p && p.status === "progress" && p.file) {
            const pct = p.total ? Math.round((p.loaded / p.total) * 100) : null;
            setLocalEngineStatus("Downloading local model (" + p.file + ")" + (pct != null ? " · " + pct + "%" : "") + " — one-time only, cached after this.");
          } else if (p && p.status === "ready") {
            setLocalEngineStatus("Local model ready.");
          }
        },
      }));
  }
  return nllbPipelinePromise;
}

function setLocalEngineStatus(text) {
  const el = $("localEngineStatus");
  if (!text) { el.style.display = "none"; el.textContent = ""; return; }
  el.style.display = "block"; el.textContent = text;
}

async function localNllbTranslate(text, src, tgt, onProgress) {
  const srcCode = NLLB_LANG_MAP[src], tgtCode = NLLB_LANG_MAP[tgt];
  if (!srcCode || !tgtCode) { const e = new Error("Language not supported by the local model"); e.code = "UNSUPPORTED"; throw e; }
  const translator = await loadNllbPipeline();
  setLocalEngineStatus("");
  const chunks = chunkText(text, 700);
  const out = [];
  for (let i = 0; i < chunks.length; i++) {
    if (onProgress) onProgress(i + 1, chunks.length);
    const result = await translator(chunks[i], { src_lang: srcCode, tgt_lang: tgtCode });
    out.push((result && result[0] && result[0].translation_text) || "");
  }
  return out.join(" ");
}

async function translateWithSelectedEngine(text, src, tgt, onProgress) {
  const useLocal = $("engineLocal").checked;
  if (!useLocal) return { out: await myMemoryTranslate(text, src, tgt, onProgress), engineNote: "" };
  try {
    const out = await localNllbTranslate(text, src, tgt, onProgress);
    return { out, engineNote: "" };
  } catch (err) {
    setLocalEngineStatus("");
    const out = await myMemoryTranslate(text, src, tgt, onProgress);
    const reason = err && err.code === "UNSUPPORTED" ? "the local model doesn't support this language pair yet" : "the local model failed to load";
    return { out, engineNote: "Fell back to MyMemory — " + reason + "." };
  }
}
...
const ENGINE_STORAGE = "palimpsest_translate_engine";
const savedEngine = localStorage.getItem(ENGINE_STORAGE);
if (savedEngine === "local") $("engineLocal").checked = true;
document.querySelectorAll('input[name="translateEngine"]').forEach(el => el.addEventListener("change", () => {
  localStorage.setItem(ENGINE_STORAGE, $("engineLocal").checked ? "local" : "mymemory");
  setLocalEngineStatus("");
}));

async function runAiRead(auto) {
  if (!$("mainImage").src) return;
  ...
  try {
    ...
    if (parsed.transcription) {
      ...
      if (!auto) html += '<button class="btn primary" id="useTranscriptionBtn" style="margin-top:8px;">Use this transcription in Selection →</button>';
    }
    ...
    // One-tap mode: when this ran automatically off a scan, don't wait for a
    // second click — drop the transcription straight into Selection and
    // translate it immediately, so "Scan image" alone produces a finished
    // result, the way a single point-and-shoot translate app would.
    if (auto && parsed.transcription) {
      $("manualText").value = parsed.transcription;
      renderSelectionText();
      await runTranslate();
      $("aiReadResult").scrollIntoView({ behavior: "smooth", block: "start" });
    }
  } catch (err) {
...
$("aiReadBtn").addEventListener("click", () => runAiRead(false));
...
async function runTranslate() {
  ...
  try {
    ...
    const { out, engineNote } = await translateWithSelectedEngine(t, src, $("targetLang").value,
    ...
    $("translationResult").innerHTML =
      ...
      (engineNote ? '<div style="font-size:12px;color:#c9a86a;margin-bottom:6px;">' + escapeHtml(engineNote) + '</div>' : '') +
